...
for p=1:size(DataSet,2)
    ...
    if floor(DataSet(p)/10) == 1
        ...
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\Intact Water\'];
    elseif floor(DataSet(p)/10) == 2
        load('PR_Intact_Cadaverine.mat')
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\Intact cadaverine\'];
    elseif floor(DataSet(p)/10) == 3
        load('PR_L-aba_Cadaverine.mat')
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\L-aba cadaverine\'];
    elseif floor(DataSet(p)/10) == 4
        load('PR_R-aba_Cadaverine.mat')
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\R-aba cadaverine\'];
    elseif floor(DataSet(p)/10) == 5
        load('PR_Bi-aba_Cadaverine.mat')
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\Bi-aba cadaverine\'];
            elseif floor(DataSet(p)/10) == 6
        load('PR_Static.mat')
        CurrentDir = [OriDir(1:end-46),'Data\pulled_2_plus_static\Static\'];
    end
...
%             dist_time{count_no_of_crossing_both_sides}=[];
            if  results.bordercrossing_details{1,i}(j,5)<zone_2 && ...
                    (results.bordercrossing_details{1,i}(j,5)>zone_1 || (results.bordercrossing_details{1,i}(j,5)==0 && results.bordercrossing_details{1,i}(j,6)>zone_1))  && ...
...
%                     dist_time{count_no_of_crossing_both_sides}=[dist_time{count_no_of_crossing_both_sides}; (k-results.bordercrossing_details{1,i}(j,1))/(results.bordercrossing_details{1,i}(j,4)-results.bordercrossing_details{1,i}(j,1)), ...
%                         FAA_ptcurvedist_samezone(data.FhCoor{1,i}(k,1),data.FhCoor{1,i}(k,2),i,j,results)];
