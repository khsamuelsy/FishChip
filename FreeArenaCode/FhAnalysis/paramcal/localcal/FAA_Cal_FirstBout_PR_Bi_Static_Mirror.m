...
for p=1:size(DataSet,2)
    ...
    for i=1:size(results.bordercrossing_details,2)
        for j=1:size(results.bordercrossing_details{1,i},1)
            ...
            if (results.bordercrossing_details{1,i}(j,6)<zone_2 && ...
                    results.bordercrossing_details{1,i}(j,6)>zone_1 && ...
...
all_latency = all_latency*1000/240;  %in ms
...
cd(olddir)
save('FirstBout_PR_Bi_Static_Mirror.mat','cal');
end
