...
DataSet = [1121,1222,40,30];
cd(path_zonelocate);zonelocate_loadborder
global border zone 
close all
for p=1:size(DataSet,2)
    ...
    for i=1:data.nfiles
        load([DataSet_names(i,1:end-9),'.mat']);
        load([DataSet_names(i,1:end-9),'_QCed.mat']);
        data.FhCoor=[data.FhCoor,FhData_QCed.FhCoor];  data.FhAng=[data.FhAng,FhData_QCed.FhAng];
        tformVec{i}=FhData.tformVec;     tformMtx{i}=FhData.tformMtx;
    ...
    for i=1:data.noffish
        ...
        for j=1:length(CoorTform(2,:))
            if sum(isnan([CoorTform(1,j),CoorTform(2,j)]))==0 && flag(CoorTform(1,j),CoorTform(2,j))==0
...
cd(currDir);
save('OverallTrajectories.mat','cal');
