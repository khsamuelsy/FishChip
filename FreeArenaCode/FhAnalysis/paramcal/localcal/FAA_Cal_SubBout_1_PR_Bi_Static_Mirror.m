...
close all
addpath('../data/')
% load('../data/cb(pulled_2)_wclass_old.mat');

load('../data/cb(pulled2)_PR_Bi_Static_Mirror.mat');
...
DataSet = [10,20,40,30,50,60];

all_latency=[]; all_dis=[];
all_boutn=[]; all_freq=[];
all_dori=[]; all_dori2=[];
all_dori_vel=[];
all_delta_angle_boutn = [];
all_delta_vel_boutn = [];
all_delta_dur_boutn  =[];
...
for p=1:size(DataSet,2)
    ...
    for i=1:size(results.bordercrossing_details,2)
        ...
        for j=1:size(results.bordercrossing_details{1,i},1)
            ...
            %             if round(results_cb_wclass{indx}.class(eventno_count+j)) ~=0
            ...
            if ~isempty(results_cb{indx}.dnormal_time{1,eventno_count+j})
                ...
                finishtime=results.bordercrossing_details{1,i}(j,4); % cell=3 for time/ distance /freq; % cell=4 for others
                ...
                for k=finishtime+1:1728000
                    ...
                    if ~isempty(find(results.allswimbout_details{1,i}(:,2)==k)) 
                        ...
                        if ~isnan(end_frame)
                            ...
                            dori_vel = [dori_vel ; delta_angle./(end_frame-start_frame).*(240./1000)];
%                             [i,j,k,indx_entry,length(dori)]
                            ...
                            delta_angle_boutn = [delta_angle_boutn ; cum_boutn, delta_angle]; % angle
                            ddisp_vel = [ddisp_vel;  (tempdisp./(end_frame-start_frame)).*(240./1000)];
                            delta_vel_boutn = [delta_vel_boutn; cum_boutn, delta_angle.*(240/1000)./(end_frame-start_frame)]; % angle velocity
                            delta_dur_boutn = [delta_dur_boutn  ; cum_boutn, (1000/240)*(end_frame-start_frame)]; % duration in ms
    ...
    dori2(end+1:10000)=NaN;    dori2=reshape(dori2,10000,1);
        dori_vel(end+1:10000)=NaN;    dori_vel=reshape(dori_vel,10000,1);
    dur(end+1:10000)=NaN;    dur=reshape(dur,10000,1);
    delta_angle_boutn(end+1:10000,:)=NaN;
    delta_vel_boutn(end+1:10000,:)=NaN;
    delta_dur_boutn(end+1:10000,:)=NaN;
    
    
    ddisp(end+1:10000)=NaN;
    ...
    all_dori2=[all_dori2,dori2];
        all_dori_vel=[all_dori_vel,dori_vel];
    all_delta_angle_boutn=[all_delta_angle_boutn,delta_angle_boutn];
    all_delta_vel_boutn=[all_delta_vel_boutn,delta_vel_boutn];
    all_delta_dur_boutn=[all_delta_dur_boutn,delta_dur_boutn];
...
cal.all_boutn = all_boutn+1; %+1 to include the first bout after decision
cal.all_dori = all_dori;
cal.all_dori2 = all_dori2;
cal.all_dori_vel = all_dori_vel;
cal.all_delta_angle_boutn=all_delta_angle_boutn;
cal.all_delta_vel_boutn=all_delta_vel_boutn;
cal.all_delta_dur_boutn=all_delta_dur_boutn;
cal.all_ddisp = all_ddisp;
cal.all_ddisp_vel = all_ddisp_vel;
cal.all_dur = all_dur;
cal.all_overall_dori = all_overall_dori;
