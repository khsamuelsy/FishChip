function FAA_BoxViolinPlot_Method(param,options)
...
if isfield(options,'set_ymax')
    param1(param1>options.set_ymax) = options.set_ymax;
end
if isfield(options,'set_ymin')
    param1(param1<options.set_ymin) = options.set_ymin;
end
...
if options.stat
    stat_param = [param(:,1:2);nan(1000000,2)];
    stat_param(:,3) = [param(:,3);param(:,4)];
    stat_param(:,4) = [param(:,5);nan(1000000,1)];
    stat_param(:,5) = [param(:,6);nan(1000000,1)];
    format shorte
...
%     
%     
%     [f,x]=ecdf(stat_param(:,i));
%     plot(x,f,'color',color_draw);
%     
% %     plot(options.set_yticks,p)
%     hold on;
% %     h{ii}.Color=color_draw;
%     
% end
