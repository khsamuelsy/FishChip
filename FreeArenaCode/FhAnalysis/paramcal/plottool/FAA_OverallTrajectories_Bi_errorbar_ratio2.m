...
for ii=1:nargin
    ...
    perc_time = sum(sum(eval(['Im_Px',num2str(ii)]).*(zone.BW_zone13 | zone.BW_zone12)));
    perc_time2 = sum(sum(eval(['Im_Px',num2str(ii)]).*(zone.BW_zone23 | zone.BW_zone22)));
...
sem = nanstd(total_perc_time)./sqrt(length(total_perc_time))
avg = nanmean(total_perc_time)
end
