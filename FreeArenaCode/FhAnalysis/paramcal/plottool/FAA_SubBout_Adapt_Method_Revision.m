...
nullcad = boutn_vs_param(:,9:10);
static = boutn_vs_param(:,11:12);
...
nullcad(isnan(nullcad(:,1)),:)=[];
static(isnan(static(:,1)),:)=[];
...
temp = bout1_vs_param(:,5);
temp2 = temp(~isnan(temp));
nullcad= [nullcad; repmat(1,length(temp2),1),temp2];
temp = bout1_vs_param(:,6);
temp2 = temp(~isnan(temp));
static = [static; repmat(1,length(temp2),1),temp2];
...
nullcad(size(nullcad,1)+1:10000,1:2) = nan;
static(size(static,1)+1:10000,1:2) = nan;

names = {'intactwater','intactcad','leftintactcad','rightintactcad','nullcad','static'};
...
FAA_Lineplot_Bi_Method_Revision(cal,options)
