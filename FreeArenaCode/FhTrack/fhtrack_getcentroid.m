function fhtrack_getcentroid(batchNum)

global gh

NumFrame = size(gh.data.ImRaw, 3);
FhCoorSze = size(gh.data.FhCoorTemp,1);
...
if batchNum == 1
    FhCoorSze = size(gh.data.FhCentroid{1,1},1);
else
    FhCoorSze = min(size(gh.data.FhCentroid{1,1},1),size(gh.data.FhCoorTemp,1));
end
