function fhtrack_trackWholeExpt_resumetrack(batchNum)
...
for k = 1:size(gh.output.FhCoor,2)
    gh.data.FhCoorTemp(k,:) = gh.output.FhCoor{end,k}(end,1:2);
    gh.data.FhCoor{1,k} = [];
end

% register images
fhtrack_imregv3_resumetrack(batchNum);
