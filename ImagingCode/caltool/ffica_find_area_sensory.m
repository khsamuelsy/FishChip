function ffica_ofind_area_sensory
...
for aa=1:length(allbrain.fishn)
    ...
    for ii=1:size(wholebrain.signal3,1)
        ...
        area.uni_r = [];
        for jj=1:size(l,1)
            area.uni_l = [area.uni_l; abs(trapz(1:actlength,l(jj,act_startf:act_endf)))];
        end
        for jj=1:size(r,1)
            area.uni_r = [area.uni_r; abs(trapz(1:actlength,r(jj,act_startf:act_endf)))];
        end
        
        wholebrain.cal2.area.uni(ii) = sum([mean(area.uni_l); mean(area.uni_r)]);
