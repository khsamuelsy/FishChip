function FFICA_BrainRegionScatter_Motor(data,options)

global mycolor allbrain rd
...
switch options.grouping
    ...
    case 'lateralized'
        ...
        for ii=1:10
            ...
            if ismember(ii,options.barset)
                ...
                mvalue(ii+10)=0;
                   ...
                   rd.is{ii} = data.qty(list_l)*-1';
            rd.is{ii+10} = data.qty(list_r)';
        end
        ...
        for ii=1:length(data.qty)
            if ~isnan(data.qty(ii)) && abs(nregionid(ii))~=1
        ...
        line(-data.fraction(2:10),2:10,'LineStyle','--','linewidth',2,'color',repmat(0,1,3),'Parent',ax2); hold on
        line(data.fraction(12:20),2:10,'LineStyle','--','linewidth',2,'color',repmat(0,1,3),'Parent',ax2)
