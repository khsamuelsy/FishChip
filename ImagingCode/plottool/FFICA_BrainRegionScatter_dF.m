function FFICA_BrainRegionScatter_dF(data,options)
        ...
        for ii=1:10
            ...
            if ~isnan(temp_mid(ii))
                if size(options.colorscheme,1)~=1
                    color_draw1= options.colorscheme(round(((temp_mid(ii)-options.set_yticks(1))/(options.set_yticks(end)-options.set_yticks(1)))*size(options.colorscheme,1)),:);
                else
